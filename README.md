...
- Punto de venta / Pago de Servicios (2026-07-24): se documenta corrección backend/BD para persistir `IMPD` por comprobante en pagos PS mixtos; frontend muestra cada cheque con su importe individual.
...
- Punto de venta / Devoluciones forma original (2026-07-06): pago devolución mantiene sección no editable y debe reflejar `TARJETA CREDITO` desde `formasSugeridas` backend, conservando referencia del origen.
...
- Facturación / Edición fiscal por folio (2026-07-30): el diálogo de validación guarda por `PATCH /facturacion/:idFol/cliente-fiscal`; mantenimiento general conserva `PATCH /factclientshp/:id`.
...
- Ordenes de trabajo / Cambio material y Merma (actualizado 2026-07-24): la captura compara precio histórico propio de la ORD contra `DAT_ART.PVTA`, permite misma ART y aplica la fracción `CTD_C_M` más IVA según `DAT_SUC`/folio.
...
- Ordenes de trabajo / Cambio material y Merma (2026-07-09): al autorizar, la UI muestra mensaje específico si la API responde error genérico; backend corrige colisión de `DAT_CTR_DOC.DOC` al asentar diferencia contable.
...
- Inventarios / Merma (2026-05-15): nuevo feature `lib/features/modulos/merma/*` con rutas separadas `/modulos/merma/gestion`, `/modulos/merma/auditoria`, `/modulos/merma/consulta` y `/modulos/merma/reportes`; Home resuelve `DAT_JAA_MERM` hacia gestión de merma y permite consultar soporte/etiqueta QR/barcode desde consulta.
...
- Inventarios / Planeacion y sugeridos de compra (2026-07-10): `DAT_JAA_SUG` navega a `/#/modulos/sugeridos`; la primera pantalla queda solo para calculos, no consulta resultados al entrar, muestra sucursales `DF01/DF04/DF05/DF06`, filtra por linea/categoria/marca/tipo de producto, crea O.C. en `ABIERTO` y exporta detalle CSV compatible con Excel.
- Inventarios / Ordenes de compra (2026-07-16): `DAT_ORD_COMP` navega a `/#/modulos/ordenes-compra`; muestra todas las O.C. con filtros, detalle y boton superior de nueva O.C. El modulo de sugeridos ya no muestra ese boton en el AppBar.
