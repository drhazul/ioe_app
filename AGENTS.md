...
- Punto de venta / Pago de Servicios (2026-07-24): frontend conserva renderizado individual de cada comprobante; la corrección de duplicación de entrega se aplica en `sp_ps_pago_finalize` y Caja General mediante SQL.
...
- Punto de venta / Devoluciones forma original (2026-07-06): la UI sigue tomando `formasSugeridas` del backend; `TARJETA CREDITO` debe mostrarse y enviarse sin edición como forma válida de devolución con referencia origen.
...
- Ordenes de trabajo / Cambio material y Merma (actualizado 2026-07-24): la nueva ORD usa `DAT_ART.PVTA`; la original usa snapshot histórico propio (`PVTA` unitario + `PVTAT` total). Siempre se calcula diferencia, incluso con misma ART, aplicando `CTD_C_M` e IVA según `DAT_SUC`/folio.
...
- Ordenes de trabajo / Cambio material y Merma (2026-07-09): en autorización final, si API devuelve error genérico la UI informa que no pudo asentar autorización; backend corrige generación de `DAT_CTR_DOC.DOC` para diferencias contables.
...
- Datos Maestros / Colaboradores compartidos por sucursal (2026-06-18): se agrega CRUD en `/#/masterdata/access/suc-colab-acceso` para parametrizar sucursal destino/origen y administrar `DAT_SUC_COLAB_ACCESO`; la pantalla se usa para compartir catálogo de colaboradores sin hardcode en ORD.
- Datos Maestros / Usuarios y Empresas (2026-06-25): se agrega CRUD `/#/masterdata/empresas`; alta/edición de usuarios reordena Sucursal/Departamento/Rol, filtra Rol por Departamento, oculta Nivel, compone Correo con empresa y sugiere usuario de cajas desde `PV_OPV.IDOPV` `5000-5999`.
...
- Ordenes de trabajo / Contexto sucursal en validaciones por código (2026-05-30): en panel y páginas directas, cuando no hay sucursal explícita confirmada para la operación, frontend ya no debe forzar `suc` del usuario (`USUARIO.SUC`) en payload `*/validar`; backend resolverá sucursal por código/`USR_MOD_SUC`.
...
- Inventarios / Planeacion y sugeridos de compra (2026-07-10): se agrega feature `lib/features/modulos/sugeridos` con ruta `/modulos/sugeridos`; Home resuelve `DAT_JAA_SUG` y la UI consume `/sugeridos` para calcular sugeridos en una pantalla inicial sin resultados precargados, con sucursales `DF01/DF04/DF05/DF06`, filtros de linea/categoria/marca/tipo de producto, crear O.C. y exportar CSV.
- Inventarios / Ordenes de compra (2026-07-16): `DAT_ORD_COMP` debe resolver a `/modulos/ordenes-compra`; este modulo lista O.C. y conserva el boton superior de nueva O.C. El AppBar de `/modulos/sugeridos` no debe mostrar boton de nueva O.C.
