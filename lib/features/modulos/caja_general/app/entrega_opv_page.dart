import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:ioe_app/core/api_error.dart';
import 'package:pdf/pdf.dart';
import 'package:pdf/widgets.dart' as pw;
import 'package:printing/printing.dart';

import 'caja_general_api.dart';
import 'caja_general_models.dart';
import 'caja_general_providers.dart';

class EntregaOpvPage extends ConsumerStatefulWidget {
  const EntregaOpvPage({
    super.key,
    required this.suc,
    required this.opv,
    required this.fecha,
    required this.tipo,
  });

  final String suc;
  final String opv;
  final DateTime fecha;
  final String tipo;

  @override
  ConsumerState<EntregaOpvPage> createState() => _EntregaOpvPageState();
}

class _EntregaOpvPageState extends ConsumerState<EntregaOpvPage> {
  static const _operationTipo = 'GLOBAL';
  bool _submitting = false;
  String _reportTipo = 'GLOBAL';
  final Map<String, TextEditingController> _impeControllers = {};

  @override
  void dispose() {
    for (final ctrl in _impeControllers.values) {
      ctrl.dispose();
    }
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final filtros = CajaGeneralFiltros(
      suc: widget.suc,
      fecha: widget.fecha,
      opv: widget.opv,
      tipo: _operationTipo,
    );
    final resumenAsync = ref.watch(cajaGeneralEntregaOpvProvider(filtros));

    return Scaffold(
      appBar: AppBar(
        title: const Text('Entrega OPV'),
        actions: [
          IconButton(
            tooltip: 'Refrescar',
            onPressed: _submitting
                ? null
                : () => ref.invalidate(cajaGeneralEntregaOpvProvider(filtros)),
            icon: const Icon(Icons.refresh),
          ),
        ],
      ),
      body: resumenAsync.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (error, _) => _ErrorBlock(
          message: apiErrorMessage(error),
          onRetry: () => ref.invalidate(cajaGeneralEntregaOpvProvider(filtros)),
        ),
        data: (resumen) {
          _syncImpeInputs(resumen.formasPago);
          final header = resumen.header;
          final isClosed = (header?.esta ?? '').trim().toUpperCase() == 'CERRADO';
          return ListView(
            padding: const EdgeInsets.all(16),
            children: [
              _HeaderCard(
                suc: widget.suc,
                opv: widget.opv,
                fecha: widget.fecha,
                tipo: _operationTipo,
                header: header,
                validation: resumen.validation,
              ),
              const SizedBox(height: 10),
              if (isClosed)
                SizedBox(
                  width: 220,
                  child: DropdownButtonFormField<String>(
                    key: ValueKey('cg-opv-report-tipo-$_reportTipo'),
                    initialValue: _reportTipo,
                    isExpanded: true,
                    decoration: const InputDecoration(
                      labelText: 'Tipo de reporte',
                      border: OutlineInputBorder(),
                      isDense: true,
                    ),
                    items: const [
                      DropdownMenuItem(value: 'GLOBAL', child: Text('GLOBAL')),
                      DropdownMenuItem(value: 'CA', child: Text('CA')),
                      DropdownMenuItem(value: 'VF', child: Text('VF')),
                    ],
                    onChanged: _submitting
                        ? null
                        : (value) {
                            if (value == null) return;
                            setState(() => _reportTipo = value);
                          },
                  ),
                ),
              if (isClosed) const SizedBox(height: 12),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: [
                  FilledButton.icon(
                    onPressed: _submitting || isClosed
                        ? null
                        : () => _cerrarEntrega(resumen),
                    icon: const Icon(Icons.lock),
                    label: const Text('Cerrar entrega'),
                  ),
                  OutlinedButton.icon(
                    onPressed: _submitting || !isClosed
                        ? null
                        : () => _reactivarEntrega(),
                    icon: const Icon(Icons.lock_open),
                    label: const Text('Reactivar entrega'),
                  ),
                  OutlinedButton.icon(
                    onPressed: _submitting ? null : _generarReporte,
                    icon: const Icon(Icons.picture_as_pdf),
                    label: const Text('Generar reporte'),
                  ),
                ],
              ),
              const SizedBox(height: 12),
              _formasTable(resumen.formasPago, isClosed: isClosed),
              const SizedBox(height: 10),
              _simpleTable(
                title: 'Resumen transacciones',
                columns: const ['AUT', 'DESC', 'CTA', 'TOTAL'],
                rows: resumen.transacciones
                    .map(
                      (row) => [
                        row.aut,
                        row.desc,
                        row.cta.toStringAsFixed(0),
                        _money(row.total),
                      ],
                    )
                    .toList(growable: false),
              ),
              const SizedBox(height: 10),
              _simpleTable(
                title: 'Venta por departamento/subdepartamento',
                columns: const ['DEPTO', 'SUBDEPTO', 'PZS', 'TOTAL'],
                rows: resumen.ventas
                    .map(
                      (row) => [
                        row.ddepa,
                        row.dsubd,
                        row.vtapzs.toStringAsFixed(2),
                        _money(row.vtapsos),
                      ],
                    )
                    .toList(growable: false),
              ),
            ],
          );
        },
      ),
    );
  }

  Widget _formasTable(List<CajaGeneralFormaPago> formas, {required bool isClosed}) {
    final totalImpt = formas.fold<double>(0, (sum, row) => sum + row.impt);
    final totalImpr = formas.fold<double>(0, (sum, row) => sum + row.impr);
    final totalImpe = formas.fold<double>(0, (sum, row) {
      final ctrl = _impeControllers[row.form];
      final currentImpe = double.tryParse(ctrl?.text ?? '') ?? row.impe;
      return sum + currentImpe;
    });
    final totalDifd = formas.fold<double>(0, (sum, row) {
      final ctrl = _impeControllers[row.form];
      final currentImpe = double.tryParse(ctrl?.text ?? '') ?? row.impe;
      return sum + (row.impt - row.impr - currentImpe);
    });

    return Card(
      elevation: 0,
      child: Padding(
        padding: const EdgeInsets.all(10),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text(
              'Formas de pago',
              style: TextStyle(fontWeight: FontWeight.w700),
            ),
            const SizedBox(height: 8),
            SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: DataTable(
                columns: const [
                  DataColumn(label: Text('FORM')),
                  DataColumn(label: Text('IMPT'), numeric: true),
                  DataColumn(label: Text('IMPR'), numeric: true),
                  DataColumn(label: Text('IMPE'), numeric: true),
                  DataColumn(label: Text('DIFD'), numeric: true),
                ],
                rows: [
                  ...formas.map((row) {
                    final ctrl = _impeControllers[row.form]!;
                    final currentImpe = double.tryParse(ctrl.text) ?? row.impe;
                    final difd = row.impt - row.impr - currentImpe;
                    return DataRow(
                      cells: [
                        DataCell(Text(row.form)),
                        DataCell(Text(_money(row.impt))),
                        DataCell(Text(_money(row.impr))),
                        DataCell(
                          SizedBox(
                            width: 110,
                            child: TextField(
                              controller: ctrl,
                              enabled: !isClosed,
                              keyboardType: const TextInputType.numberWithOptions(
                                decimal: true,
                              ),
                              decoration: const InputDecoration(
                                isDense: true,
                                border: OutlineInputBorder(),
                              ),
                              onChanged: (_) => setState(() {}),
                            ),
                          ),
                        ),
                        DataCell(Text(_money(difd))),
                      ],
                    );
                  }),
                  DataRow(
                    cells: [
                      DataCell(
                        Text(
                          'TOTALES',
                          style: const TextStyle(fontWeight: FontWeight.w700),
                        ),
                      ),
                      DataCell(
                        Text(
                          _money(totalImpt),
                          style: const TextStyle(fontWeight: FontWeight.w700),
                        ),
                      ),
                      DataCell(
                        Text(
                          _money(totalImpr),
                          style: const TextStyle(fontWeight: FontWeight.w700),
                        ),
                      ),
                      DataCell(
                        Text(
                          _money(totalImpe),
                          style: const TextStyle(fontWeight: FontWeight.w700),
                        ),
                      ),
                      DataCell(
                        Text(
                          _money(totalDifd),
                          style: const TextStyle(fontWeight: FontWeight.w700),
                        ),
                      ),
                    ],
                  ),
                ],
              ),
            ),
          ],
        ),
      ),
    );
  }

  Widget _simpleTable({
    required String title,
    required List<String> columns,
    required List<List<String>> rows,
  }) {
    return Card(
      elevation: 0,
      child: Padding(
        padding: const EdgeInsets.all(10),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(title, style: const TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: DataTable(
                columns: columns
                    .map((label) => DataColumn(label: Text(label)))
                    .toList(growable: false),
                rows: rows
                    .map(
                      (row) => DataRow(
                        cells: row
                            .map((value) => DataCell(Text(value)))
                            .toList(growable: false),
                      ),
                    )
                    .toList(growable: false),
              ),
            ),
          ],
        ),
      ),
    );
  }

  Future<void> _cerrarEntrega(CajaGeneralOpvResumen resumen) async {
    setState(() => _submitting = true);
    try {
      final entregas = resumen.formasPago
          .map(
            (forma) => CerrarEntregaFormaInput(
              form: forma.form,
              impe: double.tryParse(_impeControllers[forma.form]?.text ?? '') ??
                  forma.impe,
            ),
          )
          .toList(growable: false);
      await ref.read(cajaGeneralApiProvider).cerrarEntregaOpv(
            suc: widget.suc,
            fecha: widget.fecha,
            opv: widget.opv,
            entregas: entregas,
          );
      if (!mounted) return;
      ref.invalidate(
        cajaGeneralEntregaOpvProvider(
          CajaGeneralFiltros(
            suc: widget.suc,
            fecha: widget.fecha,
            opv: widget.opv,
            tipo: _operationTipo,
          ),
        ),
      );
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Entrega OPV cerrada correctamente.')),
      );
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(apiErrorMessage(error))),
      );
    } finally {
      if (mounted) {
        setState(() => _submitting = false);
      }
    }
  }

  Future<void> _reactivarEntrega() async {
    setState(() => _submitting = true);
    try {
      await ref.read(cajaGeneralApiProvider).reactivarEntregaOpv(
            suc: widget.suc,
            fecha: widget.fecha,
            opv: widget.opv,
          );
      if (!mounted) return;
      ref.invalidate(
        cajaGeneralEntregaOpvProvider(
          CajaGeneralFiltros(
            suc: widget.suc,
            fecha: widget.fecha,
            opv: widget.opv,
            tipo: _operationTipo,
          ),
        ),
      );
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Entrega OPV reactivada.')),
      );
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(apiErrorMessage(error))),
      );
    } finally {
      if (mounted) {
        setState(() => _submitting = false);
      }
    }
  }

  Future<void> _generarReporte() async {
    setState(() => _submitting = true);
    try {
      final reporte = await ref.read(cajaGeneralApiProvider).fetchReporteOpv(
            suc: widget.suc,
            fecha: widget.fecha,
            opv: widget.opv,
            tipo: _reportTipo,
          );
      if (!mounted) return;
      final doc = _buildReporteOpvPdf(reporte);
      final reportDate = _formatDate(widget.fecha);
      final fileName =
          'cg_opv_${widget.suc}_${widget.opv}_${_reportTipo}_$reportDate.pdf';
      final bytes = await doc.save();
      if (kIsWeb) {
        await Printing.sharePdf(bytes: bytes, filename: fileName);
      } else {
        await Printing.layoutPdf(
          name: fileName,
          onLayout: (_) async => bytes,
        );
      }
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(apiErrorMessage(error))),
      );
    } finally {
      if (mounted) {
        setState(() => _submitting = false);
      }
    }
  }

  void _syncImpeInputs(List<CajaGeneralFormaPago> formas) {
    final keys = formas.map((e) => e.form).toSet();
    final staleKeys =
        _impeControllers.keys.where((key) => !keys.contains(key)).toList();
    for (final stale in staleKeys) {
      _impeControllers.remove(stale)?.dispose();
    }
    for (final forma in formas) {
      _impeControllers.putIfAbsent(
        forma.form,
        () => TextEditingController(text: forma.impe.toStringAsFixed(2)),
      );
    }
  }

  pw.Document _buildReporteOpvPdf(Map<String, dynamic> data) {
    final header = _asMap(data['header']);
    final forms = _asMapList(data['forms']);
    final transacciones = _asMapList(data['transacciones']);
    final ventas = _asMapList(data['ventas']);
    final generatedAt = _asText(data['generatedAt']);
    final isGlobalReport = _reportTipo.toUpperCase() == 'GLOBAL';
    final totalImpt = forms.fold<double>(0, (sum, row) => sum + _asNumber(row['IMPT']));
    final totalImpr = forms.fold<double>(0, (sum, row) => sum + _asNumber(row['IMPR']));
    final totalImpe = forms.fold<double>(0, (sum, row) => sum + _asNumber(row['IMPE']));
    final totalDifd = forms.fold<double>(0, (sum, row) => sum + _asNumber(row['DIFD']));
    final formsHeaders = isGlobalReport
        ? const ['FORM', 'NOM', 'IMPT', 'IMPR', 'IMPE', 'DIFD']
        : const ['FORM', 'NOM', 'IMPT'];
    final formsRows = [
      ...forms
        .map(
          (row) => isGlobalReport
              ? [
                  _asText(row['FORM']),
                  _asText(row['NOM']),
                  _money(_asNumber(row['IMPT'])),
                  _money(_asNumber(row['IMPR'])),
                  _money(_asNumber(row['IMPE'])),
                  _money(_asNumber(row['DIFD'])),
                ]
              : [
                  _asText(row['FORM']),
                  _asText(row['NOM']),
                  _money(_asNumber(row['IMPT'])),
                ],
        ),
      if (isGlobalReport)
        [
          'TOTALES',
          '',
          _money(totalImpt),
          _money(totalImpr),
          _money(totalImpe),
          _money(totalDifd),
        ]
      else
        [
          'TOTALES',
          '',
          _money(totalImpt),
        ],
    ];

    final doc = pw.Document();
    doc.addPage(
      pw.MultiPage(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(24),
        build: (context) => [
          pw.Text(
            'REPORTE ENTREGA OPV',
            style: pw.TextStyle(
              fontSize: 16,
              fontWeight: pw.FontWeight.bold,
            ),
          ),
          pw.SizedBox(height: 6),
          pw.Text(
            'SUC: ${widget.suc}   OPV: ${widget.opv}   FCN: ${_formatDate(widget.fecha)}   TIPO: $_reportTipo',
          ),
          if (generatedAt.isNotEmpty) pw.Text('Generado: $generatedAt'),
          pw.SizedBox(height: 10),
          if (header.isNotEmpty)
            _pdfKv('Encabezado', [
              ['IDE', _asText(header['IDE'])],
              ['ESTA', _asText(header['ESTA'])],
              ['ART', _asNumber(header['ART']).toStringAsFixed(2)],
              ['TRN', _asNumber(header['TRN']).toStringAsFixed(0)],
              ['DIF', _money(_asNumber(header['DIF']))],
            ]),
          _pdfTable(
            'Formas de pago',
            formsHeaders,
            formsRows,
          ),
          _pdfTable(
            'Resumen transacciones',
            const ['AUT', 'DESC', 'CTA', 'TOTAL'],
            transacciones
                .map(
                  (row) => [
                    _asText(row['AUT']),
                    _asText(row['DESC']),
                    _asNumber(row['CTA']).toStringAsFixed(0),
                    _money(_asNumber(row['TOTAL'])),
                  ],
                )
                .toList(growable: false),
          ),
          _pdfTable(
            'Venta por departamento/subdepartamento',
            const ['DEPTO', 'SUBDEPTO', 'PZS', 'TOTAL'],
            ventas
                .map(
                  (row) => [
                    _asText(row['DDEPA']),
                    _asText(row['DSUBD']),
                    _asNumber(row['VTAPZS']).toStringAsFixed(2),
                    _money(_asNumber(row['VTAPSOS'])),
                  ],
                )
                .toList(growable: false),
          ),
        ],
      ),
    );
    return doc;
  }

  pw.Widget _pdfKv(String title, List<List<String>> rows) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
          title,
          style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11),
        ),
        pw.SizedBox(height: 4),
        ...rows.map(
          (item) => pw.Text('${item[0]}: ${item[1]}'),
        ),
        pw.SizedBox(height: 10),
      ],
    );
  }

  pw.Widget _pdfTable(
    String title,
    List<String> headers,
    List<List<String>> rows,
  ) {
    final safeRows = rows.isEmpty ? <List<String>>[List.filled(headers.length, '-')] : rows;
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
          title,
          style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11),
        ),
        pw.SizedBox(height: 4),
        pw.TableHelper.fromTextArray(
          headers: headers,
          data: safeRows,
          headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9),
          cellStyle: const pw.TextStyle(fontSize: 9),
          headerDecoration: const pw.BoxDecoration(color: PdfColors.grey300),
          border: pw.TableBorder.all(color: PdfColors.grey600, width: 0.3),
          cellPadding: const pw.EdgeInsets.symmetric(horizontal: 4, vertical: 3),
        ),
        pw.SizedBox(height: 10),
      ],
    );
  }

  Map<String, dynamic> _asMap(dynamic value) {
    if (value is Map) return Map<String, dynamic>.from(value);
    return <String, dynamic>{};
  }

  List<Map<String, dynamic>> _asMapList(dynamic value) {
    if (value is! List) return const [];
    return value
        .whereType<Map>()
        .map((item) => Map<String, dynamic>.from(item))
        .toList(growable: false);
  }

  String _asText(dynamic value) => (value ?? '').toString().trim();

  double _asNumber(dynamic value) {
    if (value is num) return value.toDouble();
    return double.tryParse((value ?? '').toString().trim()) ?? 0;
  }

  String _formatDate(DateTime value) {
    final y = value.year.toString().padLeft(4, '0');
    final m = value.month.toString().padLeft(2, '0');
    final d = value.day.toString().padLeft(2, '0');
    return '$y-$m-$d';
  }

  String _money(double value) => '\$${value.toStringAsFixed(2)}';
}

class _HeaderCard extends StatelessWidget {
  const _HeaderCard({
    required this.suc,
    required this.opv,
    required this.fecha,
    required this.tipo,
    required this.header,
    required this.validation,
  });

  final String suc;
  final String opv;
  final DateTime fecha;
  final String tipo;
  final CajaGeneralHeader? header;
  final CajaGeneralValidation? validation;

  @override
  Widget build(BuildContext context) {
    final y = fecha.year.toString().padLeft(4, '0');
    final m = fecha.month.toString().padLeft(2, '0');
    final d = fecha.day.toString().padLeft(2, '0');
    return Card(
      elevation: 0,
      child: Padding(
        padding: const EdgeInsets.all(12),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('SUC: $suc   OPV: $opv   FCN: $y-$m-$d   TIPO: $tipo'),
            if (header != null) ...[
              const SizedBox(height: 6),
              Text(
                'IDE: ${header!.ide} | TRN: ${header!.trn.toStringAsFixed(0)} | ART: ${header!.art.toStringAsFixed(2)} | DIF: \$${header!.dif.toStringAsFixed(2)} | ESTA: ${header!.esta}',
              ),
            ],
            if (validation != null) ...[
              const SizedBox(height: 6),
              Text(
                'Validación: ${validation!.status} - ${validation!.message}',
                style: TextStyle(
                  color: validation!.ok ? Colors.green.shade700 : Colors.red.shade700,
                  fontWeight: FontWeight.w600,
                ),
              ),
            ],
          ],
        ),
      ),
    );
  }
}

class _ErrorBlock extends StatelessWidget {
  const _ErrorBlock({
    required this.message,
    required this.onRetry,
  });

  final String message;
  final VoidCallback onRetry;

  @override
  Widget build(BuildContext context) {
    return Center(
      child: Padding(
        padding: const EdgeInsets.all(18),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(message),
            const SizedBox(height: 10),
            FilledButton.icon(
              onPressed: onRetry,
              icon: const Icon(Icons.refresh),
              label: const Text('Reintentar'),
            ),
          ],
        ),
      ),
    );
  }
}
