...
final facturacionPageSizeProvider = StateProvider<int>((ref) => 150);
...
final facturasPendientesProvider = FutureProvider<FacturacionPendientesPage>((
  ...
  if (selectionFilter.isNotEmpty) {
    return api.fetchPendientesByIdFols(selectionFilter.toList());
  }
...
final selectedFacturasUnificacionProvider = StateProvider<Set<String>>(
  (ref) => <String>{},
);

final facturacionFilterSucProvider = StateProvider<String>((ref) => '');
final facturacionFilterEstatusProvider = StateProvider<String>(
  (ref) => 'PENDIENTE',
);
...
final facturacionDraftFilterEstatusProvider = StateProvider<String>(
  (ref) => 'PENDIENTE',
);
final facturacionDraftFilterRazonSocialProvider = StateProvider<String>(
  (ref) => '',
);
final facturacionDraftFilterRfcReceptorProvider = StateProvider<String>(
  (ref) => '',
);
...
final facturacionDraftFilterTipoFactProvider = StateProvider<String>(
  (ref) => '',
);

final facturacionFilterInputRevisionProvider = StateProvider<int>((ref) => 0);

final facturacionIdFolSelectionFilterProvider = StateProvider<Set<String>>(
  (ref) => <String>{},
);

class FacturacionIdFolIssue {
  const FacturacionIdFolIssue({required this.idFol, required this.motivo});
...
class FacturacionIdFolSelectorNotifier
  ...
  void setValidation(List<String> validos, List<FacturacionIdFolIssue> issues) {
