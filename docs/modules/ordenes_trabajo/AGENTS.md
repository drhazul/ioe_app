...
- Home ORDs (2026-03-24): la página directa de `Entregar` exige firma digital del cliente, agrupa ORDs relacionadas en un folio de entrega y confirma el lote con backend; ya no depende de una entrega por ORD.
...
- en detalle de ORDs con `ESTSEGU=11`, el modal agrega `Imprimir evidencia` para generar PDF con cabecera, detalle, folio de entrega y firma de entrega capturada en el folio.
...
- cambio-merma integridad artículo-descripción (2026-05-29): en captura de `Nueva ORD`, si el artículo nuevo difiere del original no se permite fallback visual a la descripción original; la UI debe mostrar/guardar únicamente la descripción del artículo nuevo devuelta por backend.
- contexto sucursal en validaciones por código (2026-05-30): en `ordenes_trabajo_page.dart` y `ordenes_trabajo_action_page.dart`, `suc` solo se envía cuando hay sucursal operativa explícita (filtro, ORD relacionada o fallback puntual), evitando forzar `USUARIO.SUC` en capturas multi-sucursal.
